...
def user_logged_in_callback(sender, request, user, **kwargs):
    if user.is_staff:
        AuditLog.objects.log(request=request, action=AuditLog.Actions.LOGIN, obj=user, principal=user)


@receiver(user_logged_out)
def user_logged_out_callback(sender, request, user, **kwargs):
    if user and user.is_staff:
        AuditLog.objects.log(request=request, action=AuditLog.Actions.LOGOUT, obj=user)
