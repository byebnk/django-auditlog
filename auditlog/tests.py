...
from auditlog.signals import user_logged_in_callback
...
class UserLoggedInCallback(TestCase):
    ...
    def test_callback_successfully(self):
        ...
        self.assertTrue(AuditLog.objects.filter(principal=user).exists())
    ...
    def test_callback_with_non_staff_user(self):
        ...
        self.assertFalse(AuditLog.objects.filter(principal=db_user).exists())
    ...
    def test_callback_login_mfa(self):
        ...
        user_logged_in_callback(self.__class__, request=request, user=db_user)
        self.assertTrue(AuditLog.objects.filter(principal=db_user).exists())
